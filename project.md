...
## 177. hydra's report: a HEAD build produced no link set for five targets

Reported from hydra 2026-09-06 with the reproduction rather than a
diagnosis, since the fault is between `--eject make-fragment` and the
caller and the design question is fmake's. It is the second report from
that caller; the first is §130.

**What happened.** hydra's `tool/objsets.py` asks for a symbol closure with
`fmake -j <n> --eject make-fragment` at the repository root, and writes the
result as `test/objsets.mk`. Regeneration failed with

    fmake produced no link set for: test_bridge test_empty_state test_extloop
                                    test_probe_ui test_settings

and the caller correctly refused to write a partial file. All five sources
exist; none of them references the file that session had been editing.

**The comfortable explanation was tested and did not hold.** A killed
concurrent run corrupting the object cache was the obvious reading, so
`.fmake` was cleared entirely and the run repeated: the same five targets
failed identically. It is not the cache.

**What changed between the failing and succeeding runs was which fmake
ran.** `/usr/bin/fmake` is `fmake 1.0 (399762fd)`, mtime 2026-08-04, and
was not touched that day. The header of the file being replaced records
that it had been generated by a **scratch build of fmake HEAD dated
2026-08-25**; the successful run used the packaged 1.0. So the five
failures belong to that HEAD build rather than to the release.

**Why it is worth a report even though the release works.** The file had
been stale for long enough that a header gaining `Q_OBJECT` turned the
drift into a link error -- `moc_address_input.o` was in no object set and
every live driver failed with `undefined reference to vtable for
address_line`. A generator nobody can run stops being run, and the staleness
is the second-order cost of whatever the HEAD build did.

**What fmake may want from this end**: the five target names above, that
they produce no link set under that HEAD build from a cleared cache, and
that the packaged 1.0 handles all five. hydra holds the tree and can
re-run either build against it.

**A note on the routing, which is why this arrives late.** hydra's own
record has said since the day it happened that this was "signalled rather
than fixed from here". It was not: nothing was written into this tree, and
...
## 204. The instrument again, pointed at assembly

Three languages now. Twenty-nine assembly shapes were assembled with
`cc`, `nm` asked what each object exports, and the answer checked against
what `scan_source` recorded -- the method of §202 and §203, which is the
only part of those sections that transfers.

**Two shapes recorded nothing at all**, and both are ordinary:

    .weak default_handler      a weak definition, which the linker
    default_handler:           resolves exactly like a strong one

    .comm shared_slot,4,4      a common symbol, which needs no label

Each fails a real build the way §200 does -- `handler.s not compiled:
nothing reaches it`, then `nothing in this tree appears to define them`,
about a definition in the same directory -- and `--force-link handler.s`
builds and runs the same tree unchanged. That last step is what says the
object was always good and only the proposal was missing, which reading
the scanner cannot say.

### The half that was written because of yesterday's regression

`.weak` is not simply a third spelling beside `.globl` and `.global`.
**A `.weak` on a name the file does not define is a weak *reference*** --
the idiom that makes an optional dependency optional, where the caller
compares the symbol against zero. Reading that as a definition would
propose a file for a symbol it is asking about, and, worse, would
suppress the one line saying nothing here defines it: exactly the cost
§203 measured for `using`. So a weak name counts only when the file also
defines it, by a label or a `.set`, `.equ` or `.equiv` alias.

The case carries all three halves and each sabotage fails through its
own check: no `.comm` fails on *a common symbol was not found*, no
`.weak` on *a weak definition was not found*, and accepting every weak
name fails on the suppressed sentence.

### What the sweep found and left, with the reason

Nothing in this workspace writes either directive. **2,448 `.globl` and
246 `.global` across every `.s` and `.S` here, and no `.weak`, `.comm` or
`.lcomm` at all** -- so this is a shape found by construction rather than
by a tree that met it, and the record says so. A weak handler with a
default body is what a microcontroller's startup file is made of, which
is the tree this is for.

Two further shapes are measured and deliberately left:

- **`.symver foo, foo@@V1`** exports `foo@@V1` as well as `foo`, and a
  caller's undefined symbol would be `foo@V1`, which `symbol_tokens`
  does not split at the `@`. Nothing here writes one, and a version
  script is a shared-library concern rather than a tree-shaped one.
- **A `.globl` inside a `.macro`**, which is how libjpeg-turbo's NEON
  files export: `.globl _\fname` records the literal `_` and misses
  `sym_a` and `sym_b` entirely. This is the limit widening already
  states -- *definitions that this misses -- functions generated by
  macros, mostly -- need `--widen-all` or `--force-link*` -- and the
